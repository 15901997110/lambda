...
/*		Map<String, Optional<Book>> booksMaxPrice = books().stream()
...
/*		Collector<Book, ?, Map<String, Long>> bookMapCollector = Collectors.groupingBy(Book::getType, Collectors.counting());
		Map<String, Long> collect = books().stream().collect(bookMapCollector);
...
/*		Map<String, Optional<Book>> booksMaxPubDate = books().stream().collect(Collectors.groupingBy(Book::getType, Collectors.maxBy(Comparator.comparing(Book::getPublishDate))));
    ...
    public void test10() {
        ...
        Map<String, Map<String, Double>> collect = books.stream().collect(Collectors.groupingBy(Book::getAuthor, Collectors.groupingBy(Book::getType, Collectors.summingDouble(Book::getPrice))));
        System.out.println(collect);
    }
    private List<Book> books=books();
    private List<Book> books() {
        List<Book> books = new ArrayList<>();
        books.add(new Book(1, "tomcat", "张三", 70d, "服务器", LocalDate.parse("2014-05-17")));
        books.add(new Book(2, "jetty", "李四", 60d, "服务器", LocalDate.parse("2015-12-01")));
        books.add(new Book(3, "nginx", "张三", 65d, "服务器", LocalDate.parse("2016-10-17")));
        books.add(new Book(4, "java", "李四", 66d, "编程语言", LocalDate.parse("2011-04-09")));
        books.add(new Book(5, "ruby", "张三", 80d, "编程语言", LocalDate.parse("2013-05-09")));
        books.add(new Book(6, "php", "张三", 40d, "编程语言", LocalDate.parse("2014-08-06")));
        books.add(new Book(7, "html", "王五", 44d, "编程语言", LocalDate.parse("2011-01-06")));
        books.add(new Book(8, "oracle", "张三", 150d, "数据库", LocalDate.parse("2013-08-09")));
        books.add(new Book(9, "mysql", "王五", 66d, "数据库", LocalDate.parse("2015-04-06")));
        books.add(new Book(10, "ssh", "张三", 70d, "编程语言", LocalDate.parse("2016-12-04")));
        books.add(new Book(11, "设计模式", "李四", 81d, "其他", LocalDate.parse("2017-04-06")));
        books.add(new Book(12, "重构", "李四", 62d, "其他", LocalDate.parse("2012-04-09")));
        books.add(new Book(13, "敏捷开发", "张三", 72d, "其他", LocalDate.parse("2016-09-07")));
        books.add(new Book(14, "从技术到管理", "王五", 42d, "其他", LocalDate.parse("2016-02-19")));
        books.add(new Book(15, "算法导论", "张三", 66d, "其他", LocalDate.parse("2010-05-08")));
        books.add(new Book(16, "oracle 12c", "李四", 150d, "数据库", LocalDate.parse("2017-05-08")));
        return books;
    }
}
